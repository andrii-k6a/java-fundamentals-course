...
public class CrazyStreams {
    ...
    public BigDecimal getBalanceByEmail(String email) {
        return accounts.stream()
                .filter(account -> Objects.equals(email, account.getEmail()))
                .findAny()
                .map(Account::getBalance)
    ...
    public Map<String, BigDecimal> collectBalancesByEmailForAccountsCreatedOn(int year) {
        return accounts.stream()
                .filter(account -> year == account.getCreationDate().getYear())
    ...
    public Map<String, Set<String>> groupFirstNamesByLastNames() {
        return accounts.stream()
                .collect(Collectors.toMap(
                        Account::getLastName,
                        account -> Set.of(account.getFirstName()),
                        (s1, s2) -> {
                            s1.addAll(s2);
                            return s1;
                        }
    ...
    public Map<Character, Long> getCharacterFrequencyInFirstNames() {
        return accounts.stream()
                ...
                .mapToObj(c -> (char) c)
                .collect(Collectors.toMap(Function.identity(), c -> Long.valueOf(1), Long::sum));
    }
